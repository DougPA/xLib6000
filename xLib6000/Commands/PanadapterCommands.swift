...
extension Panadapter {
  ...
  public func requestRfGainInfo() {
    Api.sharedInstance.send("display pan rf_gain_info " + "\(streamId.hex)", replyTo: replyHandler)
  }
  ...
  private func panadapterSet(_ token: Token, _ value: Any) {
    ...
    Api.sharedInstance.send("display pan set \(streamId.hex) " + token.rawValue + "=\(value)")
  }
  /// Set a Panadapter property on the Radio
  ///
  /// - Parameters:
  ...
  private func panadapterSet(_ token: String, _ value: Any) {
    ...
    Api.sharedInstance.send("display pan set \(streamId.hex) " + token + "=\(value)")
  }
